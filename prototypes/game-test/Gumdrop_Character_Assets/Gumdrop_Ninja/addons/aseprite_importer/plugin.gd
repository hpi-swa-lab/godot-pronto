...
func _enter_tree() -> void:
	...
	interface.connect("ready", Callable(self, "_on_interface_ready").bind(), CONNECT_ONE_SHOT)

	editor_viewport.add_child(interface)
	_make_visible(false)

	connect("scene_changed", Callable(self, "_on_scene_changed"))
	editor_settings.connect("changed", Callable(self, "_on_settings_changed"))
	interface.connect("animations_generated", Callable(self, "_on_animations_generated"))
