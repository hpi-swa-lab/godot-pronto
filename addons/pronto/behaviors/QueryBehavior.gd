...
#signal found_old(node, token, priority, selection_arg)
signal found(node)
...
#signal found_all_old(nodes, token, priorities, selection_args)
signal found_all(nodes)
## Emitted if no node was found.
## [param token] The associated token with the request.
#signal found_none_old(token)
signal found_none()
...
#func query(additional_queries: Array[Query] = []):
func query():
