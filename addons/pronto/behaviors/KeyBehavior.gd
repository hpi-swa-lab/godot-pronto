...
## For special keys use 'Space' and 'Backslash' instead of ' ' or '\'
...
func _input(event):
	if event is InputEventKey and event.as_text_keycode().to_upper() == key.to_upper() and not event.echo and can_process():
...
func _get_configuration_warnings():
	if not OS.find_keycode_from_string(key): # Returns 0 if key is not valid
