...
MAXDATE=${MAXDATE:-$(date -u +"%Y-%m-%dT%H:%M:%SZ")}
MINDATE=${MINDATE:-$(date -d "$(date -d "$MAXDATE" "+%Y-%m-%d") -$(( ($(date -d "$MAXDATE" "+%u") + 7 - 4) % 7 + ($(date -d "$MAXDATE" "+%u") == 4 && $(date -d "$MAXDATE" "+%H") < 11) * 7)) days 11:00:00" "+%Y-%m-%dT%H:%M:%SZ")}
