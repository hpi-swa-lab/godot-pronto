...
| Clock | Triggers after or for a set time. Can be set to paused and to only trigger once. When looking to trigger for a specific duration the `active` property of `Unil Elapsed` needs to be enabled. This will trigger every frame until the timer is elapsed. If you wish to trigger e.g. every second (instead of every frame) you can activate `use trigger intervall` and set the `trigger interval in s` to `1`. Hint: Intervall should not be smaller than typical time between frames (delta). |
