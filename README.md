...
| StateMachineBehavior | A state machine to model behavior with different states. Add states by adding StateBehvaior children. Connect other nodes with the trigger method to make triggers available inside the state machine that can be used for state transitions. Special case: The always trigger is called every frame. |
| StateBehavior | Modelling states of a state machine, use as children of StateMachineBehavior. Transition between different states using the `on_trigger_received` signal. Perform actions in states with the `entered`, `exited` and `in_state` signals. |
