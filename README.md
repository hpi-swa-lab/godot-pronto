...
| Spawner | When triggered, spawns whatever its child nodes are at its current location in the scene. Single nodes can be spawned by modifying the optional index argument in each spawning method to just the respective child node. |
